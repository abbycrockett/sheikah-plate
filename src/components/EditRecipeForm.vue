...
Pour into a pan and bake until golden brown."
  ...
  import { saveRecipe as saveRecipeToStorage, updateRecipe as updateRecipeInStorage, loadRecipeById } from '../scripts/recipesStorage.js';
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      function initializeFromRecipe(recipe) {
        if (!recipe) return;
        ...
        hearts.value = recipe.hearts || 0;
        ingredients.value = Array.isArray(recipe.ingredients) ? recipe.ingredients.join('\n') : (recipe.ingredients || '');
        imageFile.value = null;
        if (recipe.picture) {
          if (typeof recipe.picture === 'string') {
            imagePreview.value = recipe.picture;
          } else {
            imagePreview.value = URL.createObjectURL(recipe.picture);
          }
        } else {
          imagePreview.value = '';
        }
      ...
      // Load recipe by id if needed
      async function loadAndInitRecipe(id) {
        loading.value = true;
        const recipe = await loadRecipeById(id);
        initializeFromRecipe(recipe);
        loading.value = false;
      }
  
      onMounted(async () => {
        if (!props.recipe && (props.id || route.params.id)) {
          await loadAndInitRecipe(props.id || route.params.id);
        } else if (props.recipe) {
          initializeFromRecipe(props.recipe);
        }
      });
  
      // Watch for route id changes and reload recipe
      watch(() => route.params.id, async (newId, oldId) => {
        if (newId && newId !== oldId) {
          await loadAndInitRecipe(newId);
        }
      });
  
      watch(() => props.recipe, (newRecipe) => {
        if (newRecipe) initializeFromRecipe(newRecipe);
      });
      ...
      const isEditing = computed(() => !!(props.recipe && props.recipe.id || props.id || route.params.id));
      ...
      async function handleSave() {
        ...
        const recipeToSave = {
          ...
          hearts: hearts.value,
          id: (props.recipe && props.recipe.id) || props.id || route.params.id || (Date.now().toString(36) + Math.random().toString(36).substr(2)),
          picture: imageFile.value ? imageFile.value : (currentRecipe.value ? currentRecipe.value.picture : null),
          ingredients: ingredients.value.split('\n').map(i => i.trim()).filter(Boolean),
        };
        try {
          if (isEditing.value) {
            await updateRecipeInStorage(recipeToSave);
            emit('updated', recipeToSave);
          } else {
